...
def distinct_genres():
    ...
    movies_cast = movies.select(explode(split(col("genre"),'\\|')).alias("Gengres"))
...
def top_viewed():
    ...
    movies = connections.movies_connect()
    logging.info("Read data from movies and rating dataframe and finding the top 10 most viewed movies")
