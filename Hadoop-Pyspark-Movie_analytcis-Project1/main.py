...
spark = SparkSession.buildermaster("spark://localhost:7077").appName("demo").config("spark.sql.warehouse.dir",warehouse_location) \
...
def user_rated_for_each_movie():
    user_rating_count = connections.ratings_connect()
    user_rating_count.createOrReplaceTempView("user_rating_count")
    logging.info("TO find the count of number of users who gave rating for each mvoie")
