...
select *,amount,cust_id from customers
...
--if we only use oder by in over clause
select *, 
         count(*) over(order by sales_amount desc) as total_sale_count_by_shops
from shop_sales_data;


-- if we only use oredr by in over clause
select *, 
         sum(sales_amount) over(order by sales_amount desc) as total_sale_sum_by_shops
from shop_sales_data;
...
--if we only use order by and partition by clause together
select *, 
         sum(sales_amount) over(partition by shop_id order by sales_amount desc) as total_sale_sum_by_shops
from shop_sales_data;
...
select *,
        avg(saels_amount) over (order by sales_data) as rolling_avg
from amazon_sales_data;
      
select *,
        avg(saels_amount) over(order by sales_data) as rolling_avg,
        sum(saels_amount) over(order by sales_data) as rolling_sum
from amazon_sales_data; 
