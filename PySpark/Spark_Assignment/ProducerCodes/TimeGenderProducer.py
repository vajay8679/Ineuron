...
FILE_PATH = "/home/dell/Desktop/Ajay/Ineuron/PySpark/Spark_Assignment/StreamedData/TimeGender.csv"
...
def main(topic):
    ...
    json_serializer = JSONSerializer(my_schema, schema_registry_client, timegender_to_dict)
    producer = Producer(sasl_conf())

    valuee = schema_registry_client.get_latest_version(topic + '-value').schema.schema_str
    print("Version --------------------->", valuee)
    get_timegender_instance(file_path=FILE_PATH).__str__()

    print("Producing user records to topic {}. ^C to exit.".format(topic))

    producer.poll(0.0)
    try:
        for idx, timegender in enumerate(get_timegender_instance(file_path=FILE_PATH)):
            print(timegender)
