...
FILE_PATH = "/home/dell/Desktop/Ajay/Ineuron/PySpark/Spark_Assignment/StreamedData/PatientInfo.csv"
columns=['patient_id', 'sex', 'age', 'country', 'province', 'city', 'infection_case', 'infected_by', 'contact_number', 'confirmed_date', 'released_date', 'state']
...
def get_patient_instance(file_path):
    ...
    values = {"sex": "others", "infected_by" : '0', "contact_number" : '-1', "infection_case" : "Not Known"}
    df2 = df1.fillna(value=values)
    df3 = df2.drop('deceased_date', axis = 1)
    df4 = df3.drop('symptom_onset_date', axis = 1)
...
def main(topic):
    ...
    json_serializer = JSONSerializer(my_schema, schema_registry_client, patient_to_dict)
    producer = Producer(sasl_conf())

    valuee = schema_registry_client.get_latest_version(topic + '-value').schema.schema_str
    print("Version --------------------->", valuee)
    get_patient_instance(file_path=FILE_PATH).__str__()

    print("Producing user records to topic {}. ^C to exit.".format(topic))

    producer.poll(0.0)
    try:
        for idx, patient in enumerate(get_patient_instance(file_path=FILE_PATH)):
            print(patient)
