...
FILE_PATH = "/home/dell/Desktop/Ajay/Ineuron/PySpark/Spark_Assignment/StreamedData/Policy.csv"
...
def get_policy_instance(file_path):
    ...
    df=df.iloc[:,0:]
    values = {"detail" : "Not Known", "end_date" : pd.Timestamp("today").strftime("%Y-%m-%d")}
...
def main(topic):
    ...
    json_serializer = JSONSerializer(my_schema, schema_registry_client, policy_to_dict)
    producer = Producer(sasl_conf())

    valuee = schema_registry_client.get_latest_version(topic + '-value').schema.schema_str
    print("Version --------------------->", valuee)
    get_policy_instance(file_path=FILE_PATH).__str__()

    print("Producing user records to topic {}. ^C to exit.".format(topic))

    producer.poll(0.0)
    try:
        for idx, policy in enumerate(get_policy_instance(file_path=FILE_PATH)):
            print(policy)
