...
FILE_PATH = "/home/dell/Desktop/Ajay/Ineuron/PySpark/Spark_Assignment/StreamedData/Case.csv"
...
def main(topic):
    ...
    json_serializer = JSONSerializer(my_schema, schema_registry_client, case_to_dict)
    producer = Producer(sasl_conf())

    valuee = schema_registry_client.get_latest_version(topic + '-value').schema.schema_str
    print("Version --------------------->", valuee)
    get_case_instance(file_path=FILE_PATH).__str__()

    print("Producing user records to topic {}. ^C to exit.".format(topic))

    producer.poll(0.0)
    try:
        for idx, case in enumerate(get_case_instance(file_path=FILE_PATH)):
            print(case)
